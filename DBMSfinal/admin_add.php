<?php
	...
	if(isset($_POST['add'])){
		...
		$findAut = "SELECT * FROM author WHERE author_name = '$author'";//added this
		$findResult = mysqli_query($conn, $findAut);
		if(!$findResult){
			// insert into publisher table and return id
			$insertAut = "INSERT INTO author(author_name) VALUES ('$author')";
			$insertResult = mysqli_query($conn, $insertAut);
			if(!$insertResult){
				echo "Can't add new author " . mysqli_error($conn);
				exit;
			}
			$authorid = mysql_insert_id($conn);
		} else {
			$row = mysqli_fetch_assoc($findResult);
			$authorid = $row['authorid'];
		}

		$findSup = "SELECT * FROM supplier WHERE supplier_name = '$supplier'";//added this
		...
		if(!$findResult){
			...
			$supplierid = mysql_insert_id($conn);
		} else {
			$row = mysqli_fetch_assoc($findResult);
		...
		$query = "INSERT INTO books VALUES ('" . $isbn . "', '" . $title . "', '" . $author . "', '" . $image . "', '" . $descr . "', '" . $price . "', '" . $publisherid . "','" . $supplierid . "')";
